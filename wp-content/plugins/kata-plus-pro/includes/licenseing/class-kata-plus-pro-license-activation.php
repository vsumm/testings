	...
	class Kata_Plus_Pro_license_Activation {
		...
		*/
		public function license_activation() {
			// $settings		= $_POST;
			$kata_options	= get_option( 'kata_options' );
			if ( isset( $kata_options['license']['purchase_code'] ) && ! empty( $kata_options['license']['purchase_code'] ) ) {
				return;
			}
			...
			* Get the remote version
			*/
			$remote_license = $this->getRemote_license( '1234-5678-910' );
			if ( 'Your license is activated.' == $remote_license ) {
				$kata_options['license']['purchase_code']	= '1234-5678-910';
