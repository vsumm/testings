...
// Query variables
$request = isset( $_REQUEST['s'] ) ? sanitize_text_field( $_REQUEST['s'] ) : '';
$keyword = Plugin::$instance->editor->is_edit_mode() ? 'a' : $request;
$notfound = $settings['notfound'] ? $settings['notfound'] :  __( 'Sorry, but nothing matched your search terms.', 'kata-plus' );
...
if ( $the_query->have_posts() ) :
	while ( $the_query->have_posts() ) :
		...
		if ( $x == 1 ) {
			...
			<div <?php post_class( 'kata-blog-post ' . $grid_class ); ?>>
				<div class="kata-plus-post-item">
					<?php
	...
	?>
	<h3 class="kt-notfound-search-resault"><?php echo wp_kses_post( $notfound ); ?></h3>
	<?php
	echo Kata_Plus_Frontend::search( false, null, '',  __( 'search', 'kata-plus' ), '' );
endif;
