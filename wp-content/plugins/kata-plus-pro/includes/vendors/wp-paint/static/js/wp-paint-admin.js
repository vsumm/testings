...
(function (exports, $) {
    ...
    wp.media.view.Attachment.Details.prototype._attributes =
        wp.media.view.Attachment.Details.prototype.attributes;
    ...
    wp.media.wpp_refresh_frame = function () {
        if (
            wp.media.frame.content.get() !== null &&
            wp.media.frame.content.get().collection &&
            wp.media.frame.content.get().collection.props
        ) {
    ...
    window.onload = function () {
        window.addEventListener("beforeunload", function (e) {
            if ($("#wpp-editor-frame:visible").length < 1) return undefined;
    ...
    $.initialize(".media-modal-close", function () {
        ...
        $modal_close_btn.on("click", function () {
            if ($("#wpp-editor-frame:visible").length < 1) return true;
            var editor_window = $("#wpp-editor-frame:visible")[0].contentWindow;
            if (editor_window.MP_Base_state && editor_window.MP_Base_state.layers_archive) {
                var la = editor_window.MP_Base_state.layers_archive;
                if (la.length < 1 || null == la[0]) {
    ...
    $.initialize(".edit-attachment", function () {
        ...
        if (
            ...
            $edit_attachment_button.length < 1 ||
            $wpp_edit_attachment_button.length > 0
        )
            return;
        var $wpp_edit_attachment_button = jQuery( '<a href="javascript:void(0);" class="wpp-edit-attachment" data-attachment-id="' + attachment_id + '">Edit Image PhotoShut</a>' );
        ...
        $wpp_edit_attachment_button.click(function () {
            ...
            window.wpp_open_editor(
                ...
                function (fr) {
                    ...
                    if (
                        wp.media.wpp_modal_close_refresh_url &&
                        wp.media.wpp_modal_close_refresh_url.match(/^https?:\/\//g)
                    ) {
    ...
    window.wpp_open_editor = function (attachment_id, open_callback, close_callback) {
        ...
        wpp_edit_attachment_frame.on("open", function () {
            ...
            $attachment_frame.find(".media-modal-close").attr('style', 'border-left: 1px solid #3f4248;');
            $attachment_frame.find(".media-frame-title").attr('style', 'background-color: #181A1D; border-bottom: 1px solid #3f4248;');
            $attachment_frame.find(".media-frame-title>h1").attr('style', 'display: inline-block; font-weight: 700; color: #fff');
            $attachment_frame.find(".media-frame-title>h1").html('PhotoShut<small style="margin-left: 7px; font-weight: 400; font-size: 12px; opacity: 0.7;">Powered by WP Paint Plugin</small>');
            ...
            $attachment_frame
                ...
                .html(
                    '<iframe src="' +
                        wp_paint_admin_meta.wp_admin_url +
                        "admin.php?page=wp-paint-edit-image&id=" +
                        attachment_id +
                        "&_=" +
                        Math.random() +
                        '" id="wpp-editor-frame" frameborder="0" style="width: 100%; height: 100%; border: none;"></iframe>'
                );
    ...
    var wpp_setup_attachment_edit_button = function () {
        ...
        var $wpp_edit_attachment_button = $(
            '<button type="button" class="button wpp-edit-attachment" data-attachment-id="' +
                attachment_id +
                '">Edit Image Using WP-Paint</button>'
        );
        $wpp_edit_attachment_button.click(function () {
            var $self = $(this);
            var attachment_id = $self.data("attachmentId");
