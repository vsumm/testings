	...
	public function output_by_posts($grid_preview = false)
	{
		...
		if ($do_nav) { //only do if internal is selected
			...
			foreach ($this->grid_params as $gkey => $gparam) {
				...
				// 3.0.12
				$filters_arr['filter' . $fil_id]['filter-rules'] = $base->getVar($this->grid_params, 'filter-rules' . $fil_id, 'visible');
				$filters_arr['filter' . $fil_id]['filter-rule'] = $base->getVar($this->grid_params, 'filter-rule' . $fil_id, '');
